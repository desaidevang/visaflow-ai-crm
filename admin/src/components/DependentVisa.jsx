// components/DependentVisa.jsx
import React, { useState, useEffect } from 'react';
import axios from 'axios';
import {
  Heart, Search, User, Calendar, Clock, Phone, Mail, MapPin,
  FileText, Save, Loader, CheckCircle, Edit3, ChevronRight,
  Users, X, Banknote, FileCheck, AlertCircle, Info
} from 'lucide-react';
...
const DependentVisa = ({ user, selectedLeadId, onBack }) => {
  ...
  const fetchVisaLeads = async () => {
    ...
    try {
      ...
      const visaLeads = (res.data.leads || []).filter(l => l.visa?.visaType === 'dependent_visa');
      setLeads(visaLeads);
    } catch (error) {
      console.error('Error fetching dependent visa leads:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const selectLead = (lead) => {
    ...
    setFormData({
      ...
      primaryVisaHolder: {
        name: visa.primaryVisaHolder?.name || ''
      },
      passportDetails: {
  ...
  const handleApproveVisa = async () => {
    if (!selectedLead) return;
    if (!window.confirm(`Approve dependent visa for ${selectedLead.name}?`)) return;
    ...
    try {
      ...
      alert(`✅ Dependent Visa APPROVED for ${selectedLead.name}!`);
      fetchVisaLeads();
  ...
  const handleRejectVisa = async () => {
    ...
    if (!reason) return;
    if (!window.confirm(`Reject dependent visa for ${selectedLead.name}?`)) return;
    ...
    try {
      ...
      alert(`❌ Dependent Visa REJECTED for ${selectedLead.name}`);
      fetchVisaLeads();
      setSelectedLead(null);
    } catch (error) {
      ...
      <div className="flex-shrink-0 mb-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Dependent Visa Management</h1>
              <p className="text-sm text-gray-500">Manage dependent/family visa applications</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex-1 bg-gray-50 p-6 overflow-y-auto">
          {!selectedLead ? (
            <div className="flex flex-col items-center justify-center h-full">
              <Heart className="w-16 h-16" style={{ color: RED_MID }} />
              <p className="mt-4 text-gray-500">Select a dependent visa application</p>
            </div>
          ) : (
            <div className="max-w-2xl mx-auto space-y-4">
              <div className="bg-white rounded-2xl border p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="text-xs text-gray-500">Primary Visa Holder Name</label>
                    {editing ? <input value={formData.primaryVisaHolder.name} onChange={e => handleChange('primaryVisaHolder', 'name', e.target.value)} className="w-full p-2 border rounded-lg" />
                      : <p className="p-2 bg-gray-50 rounded-lg">{formData.primaryVisaHolder.name || 'Not specified'}</p>}
                  </div>
                  <div>
                    <label className="text-xs text-gray-500">Relationship</label>
                    {editing ? <input value={formData.relationship} onChange={e => handleChange(null, 'relationship', e.target.value)} className="w-full p-2 border rounded-lg" />
                      : <p className="p-2 bg-gray-50 rounded-lg">{formData.relationship || 'Not specified'}</p>}
                  </div>
                  <div>
