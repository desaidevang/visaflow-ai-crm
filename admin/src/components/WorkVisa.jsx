// components/WorkVisa.jsx
import React, { useState, useEffect } from 'react';
import axios from 'axios';
import {
  Briefcase, Search, User, Calendar, Clock, Phone, Mail, MapPin,
  FileText, Save, Loader, CheckCircle, Edit3, ChevronRight,
  Building2, Users, X, Banknote, FileCheck, AlertCircle, Info, DollarSign
} from 'lucide-react';
...
const WorkVisa = ({ user, selectedLeadId, onBack }) => {
  ...
  const fetchVisaLeads = async () => {
    ...
    try {
      ...
      const visaLeads = (res.data.leads || []).filter(l => l.visa?.visaType === 'work_visa');
      setLeads(visaLeads);
    } catch (error) {
      console.error('Error fetching work visa leads:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const selectLead = (lead) => {
    ...
    setFormData({
      ...
      employerDetails: {
        companyName: visa.employerDetails?.companyName || '',
        jobTitle: visa.employerDetails?.jobTitle || '',
        offerLetterDate: visa.employerDetails?.offerLetterDate?.split('T')[0] || '',
        salary: visa.employerDetails?.salary || ''
      },
      passportDetails: {
        passportNumber: visa.passportDetails?.passportNumber || '',
        expiryDate: visa.passportDetails?.expiryDate?.split('T')[0] || ''
      },
      workExperience: visa.workExperience || '',
      additionalInfo: {
  ...
  const handleApproveVisa = async () => {
    if (!selectedLead) return;
    if (!window.confirm(`Approve work visa for ${selectedLead.name}?`)) return;
    ...
    try {
      ...
      alert(`✅ Work Visa APPROVED for ${selectedLead.name}!`);
      fetchVisaLeads();
  ...
  const handleRejectVisa = async () => {
    ...
    if (!reason) return;
    if (!window.confirm(`Reject work visa for ${selectedLead.name}?`)) return;
    ...
    try {
      ...
      alert(`❌ Work Visa REJECTED for ${selectedLead.name}`);
      fetchVisaLeads();
      setSelectedLead(null);
    } catch (error) {
      ...
      <div className="flex-shrink-0 mb-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Work Visa Management</h1>
              <p className="text-sm text-gray-500">Manage work/employment visa applications</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="p-4" style={{ background: `linear-gradient(135deg,${RED_DARK} 0%,${RED} 100%)` }}>
            <div className="relative">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)}
                placeholder="Search work visa applicants..."
        ...
        <div className="flex-1 bg-gray-50 p-6 overflow-y-auto">
          {!selectedLead ? (
            <div className="flex flex-col items-center justify-center h-full">
              <Briefcase className="w-16 h-16" style={{ color: RED_MID }} />
              <p className="mt-4 text-gray-500">Select a work visa application</p>
            </div>
          ) : (
              ...
              <div className="bg-white rounded-2xl border p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    {editing ? <input value={formData.employerDetails.companyName} onChange={e => handleChange('employerDetails', 'companyName', e.target.value)} className="w-full p-2 border rounded-lg" />
                      : <p className="p-2 bg-gray-50 rounded-lg">{formData.employerDetails.companyName || 'Not specified'}</p>}
                  </div>
                  <div>
                    <label className="text-xs text-gray-500">Job Title</label>
                    {editing ? <input value={formData.employerDetails.jobTitle} onChange={e => handleChange('employerDetails', 'jobTitle', e.target.value)} className="w-full p-2 border rounded-lg" />
                      : <p className="p-2 bg-gray-50 rounded-lg">{formData.employerDetails.jobTitle || 'Not specified'}</p>}
                  </div>
                  <div>
                    <label className="text-xs text-gray-500">Annual Salary</label>
                    {editing ? <input value={formData.employerDetails.salary} onChange={e => handleChange('employerDetails', 'salary', e.target.value)} className="w-full p-2 border rounded-lg" />
                      : <p className="p-2 bg-gray-50 rounded-lg">{formData.employerDetails.salary ? `$${formData.employerDetails.salary}` : 'Not specified'}</p>}
                  </div>
                  <div>
